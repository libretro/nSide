SystemSettings::SystemSettings(TabFrame* parent) : TabFrameItem(parent) {
  ...
  appendButton.onActivate([&] { systemProperties->append(); });
  modifyButton.onActivate([&] { modify(); });
  removeButton.onActivate([&] { remove(); });

  reload();
}

auto SystemSettings::refreshLocale() -> void {
  setText(locale["Settings/Systems"]);

...
auto CartPal::famicomboxManifest(vector<uint8_t>& buffer, string location) -> string {
  bool ines = true;
  if(buffer[0] != 'N'
  || buffer[1] != 'E'
  || buffer[2] != 'S'
  ...
  if(settings["icarus/UseDatabase"].boolean()) {
    ...
    for(auto cartridge : Database::FamicomBox.find("cartridge")) {
...
auto CartPal::famicomboxImport(vector<uint8_t>& buffer, string location) -> string {
  ...
  auto manifest = famicomboxManifest(buffer, location);
